...
void MapManager::BeginPlay()
{
	...
	std::shared_ptr<Player> player = level.lock()->SpawnActor<Player>(Vector2(startPosX, startPosY));
	std::shared_ptr<Cursor> cursor = level.lock()->SpawnActor<Cursor>(Vector2(startPosX, startPosY));
...
bool MapManager::FindDoorPosition(const RoomInfo* const parentA, const RoomInfo* const parentB, Craft::Vector2& outPos) const
{
	for (size_t i = 0; i < _doors.size(); i++)
	{
...
{
	if (!_instance)
		_instance = std::make_unique<MapManager>();

	return *_instance;
}

void MapManager::Split(BSPNode<Rect>& parent)
{
...
bool MapManager::IsNeighbor(const Rect& a, const Rect& b)
{
...
void MapManager::ConnectRooms(RoomInfo*& outEntrance, RoomInfo*& outExit)
{
	std::vector<BSPNode<Rect>*> leaves = _bsp->GetLeaves();
...
void MapManager::ConnectPath(std::vector<RoomInfo*>& path, int cost)
{
	std::vector<BSPNode<Rect>*> leaves = _bsp->GetLeaves();
...
bool MapManager::HasDoor(const RoomInfo* const a, const RoomInfo* const b) const
{
	for (size_t i = 0; i < _doors.size(); i++)
	{
